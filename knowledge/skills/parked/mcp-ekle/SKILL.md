...
> ve baştan sona aider'ın MCP mekanizmasını anlatıyor (`.mcp.json`, `/mcp reload`, `.aider/permissions.yml`,
> "aider" ifadeleri) — opencode'un kendi MCP yapılandırma biçimi **farklı** olabilir ve bu denetimde
> doğrulanmadı. Daha önemlisi: `knowledge/policy/PERMISSION-MATRIX.md` §1 MCP'yi "**v1'de tanımsız/yok
> (Faz 2)**" olarak işaretliyor — opencode-agent'ın bugünkü kurulumunda (`engine/opencode.json`) hiç MCP
> sunucusu tanımlı değil ve v1 kapsamında (salt-okunur, ADR-0001) MCP eklenmesi planlanmıyor. Bu beceriyi
> opencode-agent bağlamında **kullanmadan önce** opencode'un gerçek MCP config biçimini `opencode.ai` güncel
> dokümantasyonundan doğrula — burada yazılanları olduğu gibi uygulama.
