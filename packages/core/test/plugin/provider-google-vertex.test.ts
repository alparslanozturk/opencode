...
describe("GoogleVertexPlugin", () => {
  ...
  it.effect("expands endpoint templates on Vertex chat models", () =>
    Effect.gen(function* () {
      ...
      yield* catalog.transform((catalog) => {
        catalog.provider.update(Provider.ID.googleVertex, (provider) => {
          provider.package = "@opencode/ai/providers/google-vertex"
          provider.settings = { project: "config-project", location: "eu" }
        })
        catalog.model.update(Provider.ID.googleVertex, modelID, (model) => {
      ...
      yield* addPlugin()
      expect(required(yield* catalog.model.get(Provider.ID.googleVertex, modelID))).toMatchObject({
        package: "@opencode/ai/providers/google-vertex/chat",
        settings: {
