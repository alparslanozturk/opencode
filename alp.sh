...
#  önce `al.sh` ile kodu bu makineye çek, sonra burada `./alp.sh` çalıştır.
#
...
#    --bin-kopyala   derlenen ikiliyi ayrıca bin/opencode'a kopyalar (kur.sh akışı için)
...
#
#  Ortam değişkenleri:
#    KISAYOL_DIZIN        symlink dizini (varsayılan /usr/local/bin, yazılamazsa ~/.local/bin)
