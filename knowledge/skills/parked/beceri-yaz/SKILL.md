...
> opencode-agent'ta geçersiz olanlar: **"Nereye yazılır" tablosundaki üç yol** (`aider-skills/`,
> `aider/beceriler/`, `.aider/skills/`) ve **`/skills new <ad>` / `/beceri-uret` komutları** — opencode'da
> böyle bir komut yok. opencode-agent'ta gerçek konum tektir: **`knowledge/skills/approved/<ad>/SKILL.md`**
> (bkz. `MIMARI.md`); iskelet elle oluşturulur, üreten bir komut yok. Frontmatter biçimi (`name`+`description`)
> ve "description en kritik alan" ilkesi **aynen geçerlidir** — opencode da beceriyi yalnız bu satıra bakarak
> seçer.
