...
const seed = Effect.fn(function* (fn: (provider: Provider.MutableInfo) => void) {
  ...
  yield* catalog.transform((catalog) => catalog.provider.update(providerID, fn))
  return catalog
})
  ...
  it.effect("routes the OpenAI-compatible package to the native Workers AI package", () =>
    withEnv({ CLOUDFLARE_ACCOUNT_ID: "acct", CLOUDFLARE_API_KEY: "key" }, () =>
      Effect.gen(function* () {
        ...
        expect(required(yield* catalog.provider.get(providerID)).package).toBe(
  ...
  it.effect("leaves the native Workers AI package to derive its own endpoint", () =>
    withEnv({ CLOUDFLARE_ACCOUNT_ID: "acct" }, () =>
      Effect.gen(function* () {
        ...
        yield* addPlugin()
        expect(required(yield* catalog.provider.get(providerID)).settings?.baseURL).toBeUndefined()
      }),
    ),
  )
